...
const RatedMovies = () => {
    ...
    useEffect(() => {
        if(accountDetails.isLogin){
            setLoading(true);
            fetchDataFromAPI(`/account/${accountDetails.accountId}/rated/movies` , {session_id: accountDetails.sessionId})
                .then(response => {
    ...
    return (
        <div className="h-full overflow-auto scroll-hidden">
            ...
            <h3 className="text-md font-semibold text-white my-1">Rated Movies</h3>
            {loading ? <div className="h-full flex items-center justify-center"><div className="loader"></div></div> : (
                error ? <div className="h-full flex items-center justify-center text-2xl text-white font-semibold"><p>{error}</p></div> : (
                    data.length === 0 ? <div className="h-full flex items-center justify-center text-2xl text-white font-semibold h-[calc(100vh-6.125rem]"><p>No Rated Movie is Hear!</p></div> : (
